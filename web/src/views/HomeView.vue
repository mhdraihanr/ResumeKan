...
import { FileText, LayoutTemplate, PenLine, Sparkles, Download } from "lucide-vue-next";
...
const sample: CvData = {
  ...
  organizations: [],
  skills: { hard: "Vue.js, TypeScript, Tailwind CSS, Git", soft: "Komunikasi, Kerja tim" },
  ...
  <main class="bg-paper">
    ...
    <section class="border-b-2 border-ink">
      <div class="mx-auto grid max-w-6xl items-center gap-10 px-4 py-16 sm:px-6 lg:grid-cols-2 lg:py-24">
        <div>
          <Badge class="bg-powder text-ink">Generator CV ATS</Badge>
          <h1
            class="mt-4 text-4xl font-black leading-tight tracking-tight text-ink sm:text-5xl"
            v-motion
          ...
          <p class="mt-4 max-w-md text-base leading-relaxed text-ink/70">
            Isi form terstruktur, pilih template, dan unduh PDF siap lamar. Tanpa ribet, tanpa template aneh.
          </p>
        ...
        >
          <div class="overflow-hidden rounded-base border-3 border-ink bg-white shadow-[8px_8px_0_0_#0f172a]">
            <!-- Title bar -->
            <div class="flex items-center gap-2 border-b-2 border-ink bg-ink px-4 py-2.5">
              ...
              <span class="ml-3 text-xs font-medium text-paper/80">resumekan.app/preview</span>
            </div>
            <!-- Toggle template -->
            <div class="flex items-center justify-center gap-2 border-b-2 border-ink bg-paper px-4 py-2.5">
              <button
    ...
    <!-- FITUR -->
    <section class="border-b-2 border-ink bg-ink">
      <div class="mx-auto max-w-6xl px-4 py-16 sm:px-6">
        ...
        <div class="mt-8 grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
          <Card
            ...
            class="border-paper/80 bg-secondary-background"
          >
            <CardHeader>
              <div class="flex size-11 items-center justify-center rounded-base border-2 border-ink bg-powder">
                <component :is="f.icon" class="size-5 text-ink" />
              </div>
              <CardTitle class="text-lg font-black text-ink">{{ f.title }}</CardTitle>
              <CardDescription class="text-sm text-ink/70">{{ f.desc }}</CardDescription>
            </CardHeader>
          </Card>
        </div>
      </div>
    ...
    <section class="border-b-2 border-ink bg-paper">
      <div class="mx-auto max-w-6xl px-4 py-16 sm:px-6">
        <h2 class="text-3xl font-black tracking-tight text-ink">
          Dua template, satu standar ATS.
        </h2>
        <p class="mt-2 max-w-lg text-base text-ink/70">
          Modern untuk kesan segar, classic untuk kesan formal. Keduanya single-column dan siap dibaca mesin.
        </p>
        <div class="mt-8 grid gap-6 lg:grid-cols-2">
          <Card v-motion :initial="{ opacity: 0, y: 12 }" :enter="{ opacity: 1, y: 0 }">
            <CardHeader>
              <CardTitle class="text-lg font-black text-ink">Modern</CardTitle>
              <CardDescription class="text-sm text-ink/70">
                Sans-serif, header nama besar, aksen navy di judul section.
              </CardDescription>
            </CardHeader>
            <CardContent>
              <div class="max-h-64 overflow-auto rounded-base border-2 border-ink p-3">
                <CvPreview :data="sample" template="modern" />
              </div>
            </CardContent>
          </Card>
          <Card v-motion :initial="{ opacity: 0, y: 12 }" :enter="{ opacity: 1, y: 0, transition: { delay: 100 } }">
            <CardHeader>
              <CardTitle class="text-lg font-black text-ink">Classic</CardTitle>
              <CardDescription class="text-sm text-ink/70">
      ...
      <div class="mx-auto max-w-3xl px-4 py-16 text-center sm:px-6">
        ...
        <div class="mt-6">
          <RouterLink to="/register">
            <Button
              size="lg"
              class="border-ink bg-paper text-ink shadow-[4px_4px_0_0_#0f172a] hover:translate-x-boxShadowX hover:translate-y-boxShadowY hover:shadow-none"
            >
