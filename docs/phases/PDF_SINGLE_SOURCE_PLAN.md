...
**Data pribadi header setara link (2026-09-13):** setelah link beres, email/telepon/alamat non-link masih terbaca lebih lemah dari link di baris yang sama (slate-600 vs ink) padahal keduanya sama-sama jalur kontak recruiter. Ketiganya dinaikkan ke ink (`text-slate-900` / `#111`) tanpa underline, karena underline hanya untuk elemen yang benar-benar bisa diklik. Pemisah `·` tetap T4 (`text-slate-300`) sebagai dekoratif, dan baris placeholder saat kontak kosong tetap redup karena itu bukan data user. Diverifikasi lewat PDF asli: email/telepon terukur `#0f172b` (modern, classic) dan alamat Neon `#111111`, sama dengan link di dokumen yang sama, dan `screen` == `print` di browser.

**Link netral supaya preview = PDF (2026-09-13):** print CSS `a { color: inherit !important; text-decoration: none !important }` ternyata bukan hanya menetralkan warna di kertas, tapi juga **mengalahkan class warna Tailwind apa pun** pada `<a>`. Akibatnya di Browsershot link jatuh ke `inherit` = warna parent (`text-slate-600` = `#45556c`) dan underline-nya hilang, jadi preview dan PDF beda tepat di link. Perbaikannya di `CvPreview.vue`: aturan dipersempit jadi `a:not([class])` dan `!important` dicabut. Link tanpa class (tautan dalam konten user) tetap dinetralkan supaya biru default browser tidak tercetak; link template CV (semuanya punya class warna) kini benar-benar menerapkan warnanya. Link template memakai `text-slate-900` (`#0f172b`) untuk modern/classic dan `text-[#111]` untuk neon, dengan `underline decoration-slate-300 underline-offset-2`. Diverifikasi lewat PDF asli: warna teks link di PDF `#0f172b`/`#111111`, garis underline `#cad5e2`/`#9ca3af` ada sebagai vector, dan media `screen` vs `print` di browser menghasilkan nilai identik.
