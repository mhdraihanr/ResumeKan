...
- Fix: field mendapat `dark:border-border dark:bg-secondary-background dark:text-foreground dark:focus:border-ring` (MetaStep + FormInput/FormSelect/FormTextarea). Placeholder naik `65% → 80%` di `.dark .cv-form input::placeholder` (5.21:1). `FormLabel` → `dark:text-foreground/75` (6.44:1). `Hapus foto` → `dark:text-red-300` (5.44:1), error → `dark:text-red-300`, teks petunjuk foto → `dark:text-slate-400`. Buffer/override `dark:placeholder:` per-input yang mati dibuang (single-source di `.cv-form`).
- Warna error dark diselaraskan ke `red-300` di semua pesan error (inline/banner/toast, 9 file) pada audit 2026-09-15; sebelumnya `red-400` (3.78:1) gagal AA di atas kartu `#3f3f46`. Sekaligus membungkus blok `.cv-form` dalam `@layer components` (CSS unlayered menimpa utility Tailwind ber-layer tanpa peduli specificity — sempat membuat semua error dark jadi abu).
