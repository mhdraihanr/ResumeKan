...
6. **Format tetap ketat saat draft (2026-09-15):** draft melonggarkan *wajib diisi*, **bukan** format. Isi Email `budi@` dan Telepon `abc` → klik `Simpan Draft` → **tidak ada** request `POST/PUT` (dicegah klien via `checkFormats()`), error inline muncul (`Email belum valid — contoh: nama@email.com`, `Telepon hanya boleh angka dan simbol + - ( ) . serta minimal 7 digit.`), toast `Perbaiki dulu isian yang salah format.` — bukan pesan 422 mentah `The data.personal.email field must be a valid email address.`. Perbaiki jadi valid → error hilang otomatis tanpa submit ulang → `Simpan Draft` → `PUT ?draft=1` → toast `Draft tersimpan`.
...
3. **Guard server (Opsi B, 2026-09-15):** `CvController::pdf()` menolak dengan `422` + `{"message":"Lengkapi dulu sebelum mengunduh.","errors":{...}}` bila `title` atau `data.personal.{name,email,phone,address}` kosong, **atau** format email/telepon salah. Verifikasi via reflection `missingForPdf`: CV lengkap → `[]`, CV kosong → 4 entri error, email/telepon salah format → 2 entri error. Jalur editor lolos klien dulu sehingga 422 nyaris tak mungkin terpicu di UI; guard ini untuk pemanggil langsung.
