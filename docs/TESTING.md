...
6. **Format tetap ketat saat draft (2026-09-15):** draft melonggarkan _wajib diisi_, **bukan** format. Isi Email `budi@` dan Telepon `abc` → klik `Simpan Draft` → **tidak ada** request `POST/PUT` (dicegah klien via `checkFormats()`), error inline muncul (`Email belum valid — contoh: nama@email.com`, `Telepon hanya boleh angka dan simbol + - ( ) . serta minimal 7 digit.`), toast `Perbaiki dulu isian yang salah format.` — bukan pesan 422 mentah `The data.personal.email field must be a valid email address.`. Perbaiki jadi valid → error hilang otomatis tanpa submit ulang → `Simpan Draft` → `PUT ?draft=1` → toast `Draft tersimpan`.
7. **Telepon angka:** rule server `regex:/^[0-9+().\-\s]{7,30}$/` (angka + `+ - ( ) . spasi`, min 7 digit). Verifikasi: `+62 812-3456-7890` lolos, `abc` ditolak. Pesan dari `StoreCvRequest::messages()` berbahasa Indonesia (sebelumnya default Inggris karena locale `en`).
8. **Bug ruleset draft (2026-09-15):** `rules()` draft semula mengasumsikan tiap rule string lalu `explode('|')` — pecah saat rule telepon ditulis sebagai array (`['required','string','max:30','regex:...']`) → `500 TypeError`. Diperbaiki: normalkan `string|array` sebelum memproses, dan `regex:` tidak memuat `|` sehingga aman dari pemisahan pipa.
...
3. **Guard server (Opsi B, 2026-09-15):** `CvController::pdf()` menolak dengan `422` + `{"message":"Lengkapi dulu sebelum mengunduh.","errors":{...}}` bila `title` atau `data.personal.{name,email,phone,address}` kosong, **atau** format email/telepon salah, **atau** ada entri berulang setengah jadi (lihat poin 5). Verifikasi via `Cv::missingForPdf()`: CV lengkap → `[]`, CV kosong → 4 entri error, email/telepon salah format → 2 entri error.
4. **Dashboard (`PDF`):** tombol di-disable bila `cv.is_complete === false` (dari `CvResource`), dengan `title="Lengkapi dulu sebelum mengunduh PDF"` dan gaya greied (`disabled:text-slate-500 dark:disabled:text-foreground/50`, bukan opacity global — opacity membuat teks tak terbaca di dark mode). Kalau tetap dipanggil (mis. completeness berubah), `downloadPdf()` `fetch` endpoint dengan cookie → `422` → pesan error; `200` → blob diunduh lewat anchor + object URL (bukan `window.open`, tahan popup-blocker). Diuji: `CV Indo` (3 sertifikat bolong) → tombol `disabled` + tooltip; `Agustus 2026 (EN)` → enabled "Unduh PDF".
5. **Entri setengah jadi ikut memblokir (2026-09-15):** `Cv::missingForPdf()` kini juga menelusuri `ENTRY_FIELDS` (cerminan `ENTRY_RULES` klien + `required_with` server) — Pengalaman (Perusahaan/Posisi/Tanggal mulai/Tanggal selesai), Pendidikan (Institusi/Gelar/Tahun), Organisasi (Organisasi/Peran/Periode), Sertifikat (Nama/Penerbit/Tahun terbit), Proyek (Nama proyek/Peran). Aturan: entri yang **punya isi apa pun** dituntut lengkap; entri **kosong total** dilewati (sudah di-prune klien). Konsistensi yang dituju: apa yang tak bisa disimpan, tak bisa diunduh. Diuji di editor CV id=5 (3 sertifikat tanpa Penerbit/Tahun) → klik `Download PDF` → tidak ada `window.open`, toast `Lengkapi dulu sebelum mengunduh.`, pindah ke step 9 "Sertifikat", 6 error inline (`Penerbit (Sertifikat #1) wajib diisi.` dst).
6. **Catatan aksesibilitas:** error editor dari gate ini identik dengan jalur submit (banner persisten + teks di deskripsi field), bukan toast saja; pesan Dashboard memakai pola error inline halaman yang sama (`role` teks, bukan hanya warna).
