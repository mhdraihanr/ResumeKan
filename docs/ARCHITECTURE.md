...
- **Indikator A4 & Zoom Preview (2026-09-17):** Di editor (`CvFormView.vue`), header kolom preview menampilkan badge `A4 Canvas` (penanda format kertas standar cetak) dan toolbar zoom persentase terpadu. `CvPreview.vue` mendukung prop `zoom?: number | 'fit'` dan emit `scaleChange`. Fitur zoom mencakup preset kelipatan 25% (50% s/d 150%), tombol `-`/`+`, serta tombol `Fit` terpisah untuk kembali ke skala otomatis selebar kolom. Card pembungkus preview menggunakan `overflow-x-auto` dengan padding halus `p-2.5 sm:p-3.5` agar dokumen tidak terpotong saat diperbesar dan dapat di-scroll horizontal secara natural.
- **Kustomisasi Font & Skala Ukuran Teks (2026-09-18):** Step 1 (`MetaStep.vue`) menyediakan pengaturan tipografi dokumen: `fontFamily` dengan 5 kurasi ATS-friendly (`default`, `inter`, `source-sans`, `lora`, `merriweather`) dan `fontSize` dengan 3 preset skala (`compact` 10pt base, `default` 11pt base, `spacious` 12pt base). Font web non-default dimuat via Google Fonts CDN di `CvPreview.vue` dan disinkronkan dengan `await document.fonts.ready` sebelum kalkulasi page break editor dan render PDF. Skala ukuran teks ditangani lewat kelas penampung `.cv-size-compact` dan `.cv-size-spacious` di `main.css` dengan `!important` untuk menimpa utilitas `text-[10pt]` Tailwind v4 secara deterministik. Nilai tersimpan di `data.fontFamily` & `data.fontSize` dan diteruskan langsung ke `print.html` sehingga preview dan PDF selalu 100% konsisten.
...
- **ATS Keyword Tailoring (2026-09-18):** `generateSummary()` menerima parameter opsional `$jobDescription`. Jika diisi, deskripsi lowongan ditambahkan ke prompt user dan system prompt mendapat aturan ke-6: selaraskan kata kunci dengan target lowongan tanpa mengarang tools/kualifikasi baru. Prompt dipotong ke 1500 char di klien dan 3000 char di server. UI: panel collapsible "Sesuaikan dengan Target Lowongan (ATS Tailoring)" di step Ringkasan, dengan badge "Opsional" dan tombol berubah jadi "Generate dengan Lowongan" saat textarea terisi.

### ATS Compliance (2026-09-18)

Audit ATS berdasarkan riset Exa (OneResume, ATSFixer, ATS Verification, Resumefast 2025–2026):

1. **PDF berbasis teks:** `PdfService` memakai Chromium `Page.printToPDF` → vektor font + text operators asli, 100% selectable/copy-pasteable. Tidak ada rasterisasi teks. Ketajaman teks di print ditingkatkan lewat hint di `@media print` `CvPreview.vue` (`-webkit-font-smoothing: auto` + `text-rendering: geometricPrecision`) tanpa mengubah preview layar. Catatan: blur yang terlihat saat membuka PDF di browser (khususnya Edge, makin terasa saat zoom-out) adalah artefak rasterisasi canvas viewer, bukan cacat file — lihat `DESIGN.md` §4.
2. **Single-column & heading standar:** ketiga template (`CvModern`, `CvClassic`, `CvNeon`) linear single-column. Heading memakai `<h2>` dengan label standar parser ATS (`Work Experience`, `Education`, `Skills`, `Certifications`, `Projects`). Label EN `Certificates` diubah ke `Certifications` agar cocok dengan kamus baku parser Workday/Taleo.
3. **Kontak di body flow:** tag `<header>` berada di aliran utama dokumen (bukan running header Word/PDF). Browsershot tidak menginjeksi `headerTemplate`/`footerTemplate`. Kontak muncul 1× di halaman pertama — tidak ada duplication fingerprint.
4. **Badge ATS Friendly:** `CvTemplateConfig` mendapat field `atsFriendly: boolean` dan `badge?: string`. Template `modern` dan `classic` ditandai `atsFriendly: true` dengan badge "ATS Friendly" di header label selector template (editor `MetaStep.vue`) dan badge mini "ATS" di tombol toggle landing (`HomeView.vue`). Template `neon` (aksen visual + foto) tidak ditandai. Badge memakai warna emerald (kontras terverifikasi: light 6.78:1, dark 7.75:1). Label pada opsi dropdown `<select>` tetap ringkas ("Modern", "Classic", "Neon") tanpa teks panjang atau em dash agar tidak terpotong (overflow) oleh icon panah dropdown pada grid 3-kolom.
