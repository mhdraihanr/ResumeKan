...
| F2  | Form CV           | Data pribadi (termasuk foto profil opsional via upload Cloudinary), ringkasan, pengalaman kerja (employment type Full-time/Part-time/Internship, bullet per baris), pendidikan (IPK opsional, angka bold), organisasi (max 5), hard/soft skills, bahasa, sertifikat terstruktur (max 5: nama/penerbit/tahun/credential ID opsional, section sendiri), proyek terstruktur (max 8: title/role/objective/techStack/link opsional) |
